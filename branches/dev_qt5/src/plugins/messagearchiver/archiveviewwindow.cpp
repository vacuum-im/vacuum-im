...
void ArchiveViewWindow::showCollection(const ArchiveCollection &ACollection)
{
	...
	FViewOptions.selfName = FMessageStyleManager!=NULL ? FMessageStyleManager->contactName(ACollection.header.stream).toHtmlEscaped() : ACollection.header.stream.uBare().toHtmlEscaped();
