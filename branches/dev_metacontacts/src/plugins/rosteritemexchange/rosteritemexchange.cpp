...
Qt::DropActions RosterItemExchange::rosterDragStart(const QMouseEvent *AEvent, IRosterIndex *AIndex, QDrag *ADrag)
{
	...
	if (indexKind==RIK_CONTACT || indexKind==RIK_AGENT || indexKind==RIK_GROUP || indexKind==RIK_METACONTACT || indexKind==RIK_METACONTACT_ITEM)
...
QList<IRosterItem> RosterItemExchange::dragDataContacts(const QMimeData *AData) const
{
	...
	if (AData->hasFormat(DDT_ROSTERSVIEW_INDEX_DATA))
	{
		...
		else if (indexKind==RIK_CONTACT || indexKind==RIK_AGENT || indexKind==RIK_METACONTACT || indexKind==RIK_METACONTACT_ITEM)
		{
			IRosterItem ritem;
			ritem.isValid = true;
