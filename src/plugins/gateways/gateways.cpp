	...
	if (action)
	{
		...
		if (services.count() == 1)
		{
			...
			button = QMessageBox::question(NULL,tr("Remove transport and its contacts"),
				tr("Are you sure you wish to remove a transport '<b>%1</b>' and its <b>%n contact(s)</b> from the roster?","",serviceContacts(streams.first(),serviceJid).count()).arg(Qt::escape(serviceJid.domain())),
