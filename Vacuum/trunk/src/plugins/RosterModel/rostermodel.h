...
public:
  ...
  virtual IRosterIndex *createGroup(const QString &AName, IRosterIndex *AParent);
  virtual IRosterIndex *createGroup(const QString &AName, int AType, IRosterIndex *AParent);
  virtual IRosterIndex *createContact(const Jid &AJid, IRosterIndex *AParent);
  virtual IRosterIndex *findRosterIndex(int AType, const QVariant &AId, IRosterIndex *AParent=0) const;
  virtual IRosterIndex *findGroup(const QString &AName, IRosterIndex *AParent=0) const;
...
protected slots:
  ...
  void onRosterIndexDataChanged(IRosterIndex *AIndex);
private:
  IRoster *FRoster;
  IPresence *FPresence;
private:
