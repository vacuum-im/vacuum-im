  ...
  : QAbstractItemModel(parent)
{
  ...
  connect(FRootIndex,SIGNAL(childInsert(IRosterIndex *)),
    SLOT(onIndexChildInsert(IRosterIndex *)));
  connect(FRootIndex,SIGNAL(childRemove(IRosterIndex *)),
    SLOT(onIndexChildRemove(IRosterIndex *)));
}
...
IRosterIndex *RostersModel::appendStream( IRoster *ARoster, IPresence *APresence )
{
  ...
  QString streamJid = ARoster->streamJid().prep().full();
...
IRosterIndex *RostersModel::getStreamRoot( const QString &AStreamJid ) const
{
  if (FStreams.contains(AStreamJid))
    return FStreams.value(AStreamJid).root;
  return NULL;
}
...
{
  QString streamJid = ARosterItem->roster()->streamJid().prep().full();
...
void RostersModel::onSelfPresence( IPresence::Show AShow, const QString &AStatus, qint8 APriority, const Jid &AToJid )
{
  ...
  Q_ASSERT(presence);
  QString streamJid = presence->streamJid().prep().full();
  Q_ASSERT(FStreams.contains(streamJid));
  StreamItem streamItem = FStreams.value(streamJid);
  IPresence *streamPresence = streamItem.presence;
  IRosterIndex *streamRoot = streamItem.root;
...
{
  QString streamJid = APresenceItem->presence()->streamJid().prep().full();
  Q_ASSERT(FStreams.contains(streamJid));
  StreamItem streamItem = FStreams.value(streamJid);
  IRoster *streamRoster = streamItem.roster;
  IPresence *streamPresence = streamItem.presence;
...
void RostersModel::onIndexChildInsert( IRosterIndex *AIndex )
{
  ...
  if (parentIndex)
  {
    ...
    connect(AIndex->instance(),SIGNAL(dataChanged(IRosterIndex *)),
      SLOT(onIndexDataChanged(IRosterIndex *)));
    connect(AIndex->instance(),SIGNAL(childInsert(IRosterIndex *)),
      SLOT(onIndexChildInsert(IRosterIndex *)));
    connect(AIndex->instance(),SIGNAL(childRemove(IRosterIndex *)),
      SLOT(onIndexChildRemove(IRosterIndex *)));
...
void RostersModel::onIndexChildRemove( IRosterIndex *AIndex )
{
  IRosterIndex *parentIndex = AIndex->parentIndex();
  if (parentIndex)
  {
    ...
    disconnect(AIndex->instance(),SIGNAL(childInsert(IRosterIndex *)),
      this, SLOT(onIndexChildInsert(IRosterIndex *)));
    disconnect(AIndex->instance(),SIGNAL(childRemove(IRosterIndex *)),
      this, SLOT(onIndexChildRemove(IRosterIndex *)));
